 ...
 * Description: Add bbpress login link, register links, on forum pages or topic pages so users can use our forums more easier. It is super welcome to submit feature request or any question at our <a href='http://bbp.design/forums/'>Support Forum</a>. Like our plugin? <a href='https://wordpress.org/support/view/plugin-reviews/bbpress-login-register-links-on-forum-topic-pages/'>Submit a review</a>
...
function bbpressProMenuPanel()
{
	...
	add_submenu_page('bbPressCustom', __('Register Form','tomas-bbpress-custom'), __('Register Form','tomas-bbpress-custom'), 'manage_options', 'bbPressCustomRegisterForm', 'tomas_bbPressCustomRegisterForm'); //3.2.3
	//3.2.7
	add_submenu_page('bbPressCustom', __('Login Notification','tomas-bbpress-custom'), __('Login Notification','tomas-bbpress-custom'), 'manage_options', 'bbplogincustomprologinnotification', 'tomas_bbPressCustomLoginNotification');
...
function bbpressLoginRegisterLinksOnForumPage()
{
	...
	if (!(empty($tomas_trim_bbpress_custom_links_login)))
	{
		if ( !is_user_logged_in() )
		{
			...
			echo "<a href='".esc_url($login_url)."' class='bbpressloginurl'>".__('Log In','tomas-bbpress-custom').'</a> ';
			...
			echo " <a href='".esc_url($register_url)."' class='bbpressregisterurl'>".__('Register','tomas-bbpress-custom') .'</a> ';
			

			//!!! start 3.1.4
			$tomas_bbpress_login_bar_display_lost_password = get_option('tomas_bbpress_login_bar_display_lost_password');
			if ($tomas_bbpress_login_bar_display_lost_password == 'NO')
			{
			...
			else
			{
				...
				//3.1.9
				echo " <a href='".esc_url($lost_password_url)."' class='bbpresslostpasswordurl'>". __('Lost Password','tomas-bbpress-custom').'</a> ';
		...
		else
		{
			...
			//esc_url
			//3.1.9
			echo "<a class='bbpresslogouturl' href='".esc_url($logout_url)."' class='bbpresslogouturl'>".__('Log Out','tomas-bbpress-custom') .'</a> ';
	...
	else
	{
		...
		if ( !is_user_logged_in() )
		{
			...
			$tomas_bbpress_custom_links_login = $login_url;
			...
			{
				//$lost_password_url = get_option('siteurl').'/'.$tomas_bbpress_custom_links_login.'?action=lostpassword';
			    $lost_password_url = $tomas_bbpress_custom_links_login.'?action=lostpassword';
				//3.1.9
				echo " <a href='".esc_url($lost_password_url)."' class='bbpresslostpasswordurl'>". __('Lost Password','tomas-bbpress-custom').'</a> ';
				//echo " <a href='$lost_password_url' class='bbpresslostpasswordurl'>". __('Lost Password','tomas-bbpress-custom').'</a> ';
			}
		...
		else
		{
		     ...
		     */
			?>
			<a  class='bbpresscustomprofileurl' href="<?php esc_url(bbp_user_profile_url($bbpress_logged_in_user_id)); ?>" title="<?php esc_attr(bbp_displayed_user_field( 'display_name' )); ?>" rel="me">
...
function bbPressCustomMenu()
{
	...
	if (isset($_POST['bpoptionsettinspanelsubmit']))
	{
		...
		if (isset($_POST['bbpresscustomcss']))
		{
			...
			$bbpressCustomCSS = sanitize_text_field($_POST['bbpresscustomcss']); // !!!3.0.9
...
<?php 
// echo $bbpressCustomCSS;
//3.2.1
echo esc_html($bbpressCustomCSS);
?>
</textarea>
										<p><font color="Gray"><i>
										<?php 
...
function tomas_bbPressLoginAdminBar()
{
	...
	if ((isset($_POST['tomas_bbpress_submit_admin_bar'])) && (!(empty($_POST['tomas_bbpress_submit_admin_bar']))))
	{
		...
		{
			//3.2.1 $tomas_bbpress_login_admin_bar = $_POST['tomas_bbpress_login_admin_bar'];
		    $tomas_bbpress_login_admin_bar = sanitize_text_field($_POST['tomas_bbpress_login_admin_bar']);
...
function tomas_bbPressCustomLoginRedirect()
{
	...
	if ((isset($_POST['bbpress_login_redirect_free_submit'])) && (!(empty($_POST['bbpress_login_redirect_free_submit']))))
	{
		...
		if ((isset($_POST['tomas_bbpress_login_redirect'])) && (!(empty($_POST['tomas_bbpress_login_redirect']))))
		{
		    ...
		    $bbpress_login_redirect_free = sanitize_text_field($_POST['tomas_bbpress_login_redirect']);
	...
	if ((isset($_POST['bbpress_login_enable_bbpress_sidebar_submit'])) && (!(empty($_POST['bbpress_login_enable_bbpress_sidebar_submit']))))
	{
		...
		{
		    //3.2.1
		    $bbpress_enable_bbpress_sidebar_free = sanitize_text_field($_POST['tomas_bbpress_only_sidebar']);
	...
	if ((isset($_POST['bbpress_login_enable_bbpress_selected_sidebar_submit'])) && (!(empty($_POST['bbpress_login_enable_bbpress_selected_sidebar_submit']))))
	{
		...
		{
		    //3.2.1
		    $bbpress_selected_bbpress_sidebar_free = sanitize_text_field($_POST['tomas_bbpress_only_selected_sidebar']);
	...
	if ((isset($_POST['tomas_bbpress_login_bar_location_submit'])) && (!(empty($_POST['tomas_bbpress_login_bar_location_submit']))))
	{
		...
		{
		    //3.2.1
		    $tomas_bbpress_login_bar_location_free = sanitize_text_field($_POST['tomas_bbpress_login_bar_location']);
	...
	// !!! start 3.1.4
	if ((isset($_POST['tomas_bbpress_login_bar_display_lost_password_submit'])) && (!(empty($_POST['tomas_bbpress_login_bar_display_lost_password_submit']))))
	{
		//3.3.3
		check_admin_referer( 'tomas_bbpress_login_bar_disable_all_feature_submit_free_nonce' );
		if ((isset($_POST['tomas_bbpress_login_bar_display_lost_password'])) && (!(empty($_POST['tomas_bbpress_login_bar_display_lost_password']))))
		{
		    //3.2.1
		    $tomas_bbpress_login_bar_display_lost_password = sanitize_text_field($_POST['tomas_bbpress_login_bar_display_lost_password']);
			//$tomas_bbpress_login_bar_display_lost_password = $_POST['tomas_bbpress_login_bar_display_lost_password'];
			update_option('tomas_bbpress_login_bar_display_lost_password',$tomas_bbpress_login_bar_display_lost_password);
			$tomas_bbpress_MessageString =  __( 'Your changes of "Display Lost Password link" has been saved.', 'tomas-bbpress-custom' );
			tomas_bbPressCustomMessage($tomas_bbpress_MessageString);
		}
	}
	// !!! end 3.1.4
	...
	if ((isset($_POST['tomas_bbpress_login_bar_disable_all_feature_submit'])) && (!(empty($_POST['tomas_bbpress_login_bar_disable_all_feature_submit']))))
	{
		...
		{
		    //3.2.1
		    $tomas_bbpress_login_bar_disable_all_feature = sanitize_text_field($_POST['tomas_bbpress_login_bar_disable_all_feature']);
	...
	$tomas_bbpress_login_bar_display_lost_password = get_option('tomas_bbpress_login_bar_display_lost_password');  // 3.1.4
	?>
											...
											if ((isset($wp_registered_sidebars)) && (is_array($wp_registered_sidebars)) && (count($wp_registered_sidebars) >0))
											{
												foreach ($wp_registered_sidebars as $sidebarid => $sidebarattribute)
												{
													...
													if ($bbpress_selected_bbpress_sidebar_free == $sidebarattribute['id'])
													{
													     ...
													     */
														?>
														<option selected = "selected"  value="<?php echo esc_attr($sidebarattribute['id']); ?>"><?php echo esc_attr($sidebarattribute['name']); ?></option>
														<?php
													}
													else 
													{
													     ...
													     */
													?>
														<option value="<?php echo esc_attr($sidebarattribute['id']); ?>"><?php echo esc_attr($sidebarattribute['name']); ?></option>
													<?php
													}
												}
											}
...
<?php //!!!start ?>								
								...
								<?php  // !!! start 3.1.4  ?>
									...
									<span>
									<?php 
											echo  __( 'Display "Lost Password" link in the bbPress login bar?', 'tomas-bbpress-custom' );
									?>
									</span>
									</h3>
									<div class="inside" style='padding-left:10px;'>
										...
										<td scope="row"  width="40%" style="padding: 20px; text-align:left;">
										<?php 
											echo  __( 'Display "Lost Password" link?', 'tomas-bbpress-custom' );
										?>
										</td>
										<td width="60%" style="padding: 20px;">
										<select name = "tomas_bbpress_login_bar_display_lost_password" id = "tomas_bbpress_login_bar_display_lost_password">
										<?php 
											$tomas_bbpress_login_bar_display_lost_password = get_option('tomas_bbpress_login_bar_display_lost_password');
											if ($tomas_bbpress_login_bar_display_lost_password == 'NO')
											{
												?>
												<option value="YES">Display "Lost Password" in the bbPress login bar</option>
												<option selected = "selected" value="NO">Hide "Lost Password" in the bbPress login bar</option>
												<?php 
											}
											else 
											{
										...
										?>
												<option selected = "selected" value="YES">Display "Lost Password" in the bbPress login bar</option>
												<option value="NO">Hide "Lost Password" in the bbPress login bar</option>
										<?php 
											}
										?>
										</select>
										...
										<br />
										<input type="submit" id="tomas_bbpress_login_bar_display_lost_password_submit" name="tomas_bbpress_login_bar_display_lost_password_submit" value=" Submit " style="margin:1px 20px;">
										</form>
										<br />
									</div>
								</div>
							...
							<div class="postbox-container" style="width:98%;">
								<div class="postbox">
									...
									<div class="inside" style='padding-left:10px;'>
										...
										*/
										?>
										<span><font color='gray'><?php echo get_option('siteurl').'/'; ?></font></span> <input type="text" id="tomas_bbpress_custom_links_login" name="tomas_bbpress_custom_links_login" size='10' value="<?php  echo esc_attr($tomas_bbpress_custom_links_login); ?>"> <font color='gray'>/</font>
										</td>
										</tr>
...
//3.2.3
function tomas_bbPressCustomRegisterForm()
{
    if ((isset($_POST['tomas_bbpress_stop_register_spam_submit'])) && (!(empty($_POST['tomas_bbpress_stop_register_spam_submit']))))
    {
        check_admin_referer( 'tomas_bbpress_custom_links_form_submit_free_nonce' ); //!!!
        if ((isset($_POST['tomas_bbpress_stop_register_spam'])) && (!(empty($_POST['tomas_bbpress_stop_register_spam']))))
        {
            //!!! $tomas_bbpress_custom_links_login = $_POST['tomas_bbpress_custom_links_login'];
            
            $tomas_bbpress_stop_register_spam = sanitize_text_field($_POST['tomas_bbpress_stop_register_spam']);
            //if (!(empty(trim($tomas_bbpress_custom_links_login))))
            $tomas_bbpress_stop_register_spam = trim($tomas_bbpress_stop_register_spam);
            if (!(empty($tomas_bbpress_stop_register_spam)))
            {
                update_option('tomas_bbpress_stop_register_spam',$tomas_bbpress_stop_register_spam);
            }
            $tomas_bbpress_MessageString =  __( 'Your changes of "Register Form" has been saved.', 'tomas-bbpress-custom' );
            tomas_bbPressCustomMessage($tomas_bbpress_MessageString);
        }
        else
        {
    ...
    $tomas_bbpress_stop_register_spam = get_option('tomas_bbpress_stop_register_spam');
    ?>
...
<div style='padding-top:5px; font-size:22px;'>bbPress Register Form Settings:</div>
</div>
<div style='clear:both'></div>
						...
						<div id="dashboard-widgets-main-content">
							<div class="postbox-container" style="width:98%;">
								<div class="postbox">
									...
									<span>
									<?php 
											echo  __( 'Add a dynamic input field to the register form to stop a few kinds of spam-bot: ', 'tomas-bbpress-custom' );
									?>
									</span>
									</h3>
									...
									<div class="inside" style='padding-left:10px;'>
										...
										<td width="30%" style="padding: 20px;">
										<?php 
											echo  __( 'Stop a few kinds of spam?', 'tomas-bbpress-custom' );
										?>
										</td>
										<td width="70%" style="padding: 20px;text-align:left;">
										<select name = "tomas_bbpress_stop_register_spam" id = "tomas_bbpress_stop_register_spam">
										<?php
										if ($tomas_bbpress_stop_register_spam == 'No')
										{
										    ?>
												<option selected = "selected" value="No">No</option>
												<?php 
										...
										if ($tomas_bbpress_stop_register_spam == 'Yes')
										{
										    ?>
												<option selected = "selected" value="Yes">Yes</option>
												<?php 
										...
										<input type="submit" id="tomas_bbpress_stop_register_spam_submit" name="tomas_bbpress_stop_register_spam_submit" value=" Submit " style="margin:1px 20px;">
										</form>
...
if (!(empty($tomas_bbpress_stop_register_spam)))
{
    if (strtolower($tomas_bbpress_stop_register_spam) == 'yes')
    {
        add_action('register_form', 'custom_registration_form');
        //add_action('user_register', 'custom_user_register');
        add_filter('registration_errors', 'custom_user_register',10,3);
        
    }
}
//3.2.5
function custom_registration_form() {
    $field_name = 'custom_field_' . uniqid();
    echo '<input type="hidden" name="field_name" value="'.$field_name.'">';
    ?>
     <p>
         <label for="<?php echo $field_name; ?>">Real humans do not fill in this field</label>
         <input type="text" name="<?php echo $field_name; ?>" id="<?php echo $field_name; ?>" class="input" value="" size="25" tabindex="30" />
     </p>
     <?php
 }

 function custom_user_register($errors,$sanitizzed_user_login,$user_mail)
 {
     $field_name = isset($_POST['field_name'    ]) ? $_POST['field_name'] : '';
     if(!empty($field_name)){
         if(!empty($_POST[$field_name]))
         {
             $message = 'You have filled in the fields that humans should not';
 ...
 //3.2.7
 function tomas_bbPressCustomLoginNotification()
 {
...
function tomas_bbPressCustomLogin_IsAdmin($userID)
{
...
function tomas_bbPressCustomLoginAdminLoggedIn($user_login, $user)
{
    
    $var_is_bbpress_admin = tomas_bbPressCustomLogin_IsAdmin($user->ID);
...
{
    add_action( 'wp_login', 'tomas_bbPressCustomLoginAdminLoggedIn', 10, 2 );
}
